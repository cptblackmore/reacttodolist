...
function TaskList({ storageKey }) {
  ...
  const [
    tasks,
    { set: setTasks, undo: undoTasks, redo: redoTasks, canUndo, canRedo },
  ] = useUndo([...initialTasks]);
  useEffect(() => {
  ...
  function editTask(body, setIsEntered) {
    if (body) {
      ...
      setTimeout(() => {
        setTasks(
          tasks.present.map((task) =>
            task.id !== taskToEdit.id ? task : { ...task, body },
          ),
        );
  ...
  function handleDragEnd(result) {
    ...
    const sourceIndex = tasks.present.findIndex(
      (task) => task.id === filteredTasks[result.source.index].id,
    );
    const destinationIndex = tasks.present.findIndex(
      (task) => task.id === filteredTasks[result.destination.index].id,
    );
            ...
            <main className="tasks" onKeyDown={handleKeyDown}>
              {filteredTasks.length !== 0 ? (
                filteredTasks.map((task, index) => {
                  return (
                    <div
                      className={`task ${isSeparatorEntered ? "separator-entered" : ""}`}
                      key={task.id}
                    >
                      <Task
                        index={index}
                ...
                <div
                  role="alert"
                  className={`emptyState ${isEmptyStateEntered ? "entered" : ""}`}
                >
                  <EmptyState />
                  Задачи не найдены...
                </div>
